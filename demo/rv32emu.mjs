...
[ "_main", "__emscripten_thread_init", "__emscripten_thread_exit", "__emscripten_thread_crashed", "__emscripten_thread_mailbox_await", "__emscripten_tls_init", "_pthread_self", "checkMailbox", "establishStackSpace", "invokeEntryPoint", "PThread", "_calloc", "___indirect_function_table", "_realloc", "___emscripten_embedded_file_data", "_emscripten_builtin_free", "__emscripten_proxy_main", "___libc_free", "___libc_malloc", "_fflush", "_emscripten_builtin_malloc", "__ZdaPv", "__ZdaPvm", "__ZdlPv", "__ZdlPvm", "__Znaj", "__ZnajSt11align_val_t", "__Znwj", "__ZnwjSt11align_val_t", "___libc_calloc", "___libc_realloc", "_malloc_size", "_malloc_usable_size", "_reallocf", "___start_em_asm", "___stop_em_asm", "onRuntimeInitialized" ].forEach(prop => {
...
var args = [ "-k", "Image", "-i", "rootfs.cpio", "-b", "minimal.dtb" ];
...
if (Module["locateFile"]) {
 wasmBinaryFile = "rv32emu.wasm";
 if (!isDataURI(wasmBinaryFile)) {
  wasmBinaryFile = locateFile(wasmBinaryFile);
 }
} else {
 wasmBinaryFile = new URL("rv32emu.wasm", import.meta.url).href;
}
...
var ASM_CONSTS = {
 ...
 13134055: () => window.innerWidth,
 13134085: () => window.innerHeight
};
...
var PThread = {
 ...
 allocateUnusedWorker() {
  var worker;
  if (!Module["locateFile"]) {
   worker = new Worker(new URL("rv32emu.worker.js", import.meta.url), {
    type: "module"
   });
  } else {
   var pthreadMainJs = locateFile("rv32emu.worker.js");
...
function xterm_pty_old_newselect(nfds, readfds, writefds, exceptfds, timeout) {
...
var PTY_waitForReadableWithAtomicImpl = function(atomicIndex) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(4, 0, atomicIndex);
...
var PTY_waitForReadableWithAtomic = callback => {
 ...
 GROWABLE_HEAP_I32()[PTY_atomicIndex] = -1;
...
var ___syscall__newselect = (nfds, readfds, writefds, exceptfds, timeout) => (PTY_wrapPoll(() => xterm_pty_old_newselect(nfds, readfds, writefds, exceptfds, timeout)));
...
function _eglBindAPI(api) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(16, 1, api);
...
function _eglChooseConfig(display, attrib_list, configs, config_size, numConfigs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(17, 1, display, attrib_list, configs, config_size, numConfigs);
...
function _eglCreateContext(display, config, hmm, contextAttribs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(18, 1, display, config, hmm, contextAttribs);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglCreateWindowSurface(display, config, win, attrib_list) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 1, display, config, win, attrib_list);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglDestroyContext(display, context) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(20, 1, display, context);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglDestroySurface(display, surface) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 1, display, surface);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglGetConfigAttrib(display, config, attribute, value) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(22, 1, display, config, attribute, value);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglGetDisplay(nativeDisplayType) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 1, nativeDisplayType);
 EGL.setErrorCode(12288);
...
function _eglInitialize(display, majorVersion, minorVersion) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(25, 1, display, majorVersion, minorVersion);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglMakeCurrent(display, draw, read, context) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(26, 1, display, draw, read, context);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglQueryString(display, name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(27, 1, display, name);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglSwapBuffers(dpy, surface) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(28, 1, dpy, surface);
...
function _eglSwapInterval(display, interval) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 1, display, interval);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
function _eglTerminate(display) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 1, display);
 if (display != 62e3) {
  EGL.setErrorCode(12296);
  /* EGL_BAD_DISPLAY */ return 0;
 }
...
/** @suppress {duplicate } */ function _eglWaitClient() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 1);
 EGL.setErrorCode(12288);
 /* EGL_SUCCESS */ return 1;
}
...
function _eglWaitNative(nativeEngineId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(32, 1, nativeEngineId);
 EGL.setErrorCode(12288);
 /* EGL_SUCCESS */ return 1;
}
...
function getCanvasSizeMainThread(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 1, target, width, height);
...
function setCanvasElementSizeMainThread(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 1, target, width, height);
...
function _emscripten_exit_fullscreen() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(33, 1);
...
function _emscripten_get_gamepad_status(index, gamepadState) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(39, 1, index, gamepadState);
...
function _emscripten_get_screen_size(width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(41, 1, width, height);
...
function _emscripten_request_fullscreen_strategy(target, deferUntilInEventHandler, fullscreenStrategy) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(42, 1, target, deferUntilInEventHandler, fullscreenStrategy);
 var strategy = {
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(43, 1, target, deferUntilInEventHandler);
 target = findEventTarget(target);
 if (!target) return -4;
 if (!target.requestPointerLock) {
  return -1;
...
function _emscripten_set_beforeunload_callback_on_thread(userData, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(45, 1, userData, callbackfunc, targetThread);
...
function _emscripten_set_element_css_size(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(47, 1, target, width, height);
 target = findEventTarget(target);
 if (!target) return -4;
...
function _emscripten_set_gamepadconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(50, 1, userData, useCapture, callbackfunc, targetThread);
 if (!navigator.getGamepads || disableGamepadApiIfItThrows()) return -1;
 return registerGamepadEventCallback(2, userData, useCapture, callbackfunc, 26, "gamepadconnected", targetThread);
}

function _emscripten_set_gamepaddisconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(51, 1, userData, useCapture, callbackfunc, targetThread);
 if (!navigator.getGamepads || disableGamepadApiIfItThrows()) return -1;
...
function _emscripten_set_mouseenter_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(56, 1, target, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_window_title(title) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(68, 1, title);
...
var _environ_get = function(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(69, 1, __environ, environ_buf);
 var bufSize = 0;
...
var _environ_sizes_get = function(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(70, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(71, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
function xterm_pty_old_fd_read(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(72, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(73, 1, fd, offset_low, offset_high, whence, newOffset);
 var offset = convertI32PairToI53Checked(offset_low, offset_high);
 try {
  if (isNaN(offset)) return 61;
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_write(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(74, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
var ___emscripten_embedded_file_data = Module["___emscripten_embedded_file_data"] = 13051788;
...
args = ['-k', 'Image', '-i', 'rootfs.cpio', '-b', 'minimal.dtb'];
