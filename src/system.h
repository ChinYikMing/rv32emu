...
#define MMIO_OP(io, rw)                                                          \
    switch(io){                                                                  \
        ...
        case MMIO_VIRTIOBLK:                                                     \
            ...
            break;                                                               \
        case MMIO_VIRTIOVSOCK:                                                     \
            IIF(rw)( /* read */                                                  \
                mmio_read_val = virtio_vsock_read(PRIV(rv)->vsock, addr & 0xFFFFF); \
                emu_update_vsock_interrupts(rv);                                  \
                return mmio_read_val;                                            \
                ,    /* write */                                                 \
                virtio_vsock_write(PRIV(rv)->vsock, addr & 0xFFFFF, val);           \
                emu_update_vsock_interrupts(rv);                                  \
                return;                                                          \
            )                                                                    \
            break;                                                               \
...
void emu_update_vsock_interrupts(riscv_t *rv);
