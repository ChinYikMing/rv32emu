...
static off_t map_file(char **ram_loc, const char *name, off_t max_size)
{
    ...
    gzFile gzfile = NULL;
    int gzerrno;
    const char *gz_err_str = NULL;
    uint32_t dec_buf_off = 0;
    uint8_t buf[BUFSIZ];
    ...
    if (is_gzip_image) {
        ...
        while ((bytes_read = gzread(gzfile, buf, sizeof(buf))) > 0) {
            ...
            if (max_size > 0 && dec_buf_off + bytes_read > max_size) {
                rv_log_error(
                    "Decompression overflows, required: %d, quota: %ld",
                    bytes_read, (long) (max_size - dec_buf_off));
                gz_err_str = "Decompression overflows";
                goto gz_cleanup;
            }
        ...
        if (bytes_read < 0) {
            gz_err_str = gzerror(gzfile, &gzerrno);
            goto gz_cleanup;
        }
